...
while($column_email = mysqli_fetch_array($run_email)){
 ...
 while($column_refer = mysqli_fetch_array($run_refer)){
  ...
  if (empty($_POST["username"])) {
   ...
   if(in_array($username,$column1)){
    ...
    if ($referal_code == $column_refer) {
      ...
      $referal_code_err = "<div class='alert alert-primary'>
  ...
  if (empty($_POST["wallet"])) {
    ...
    $country_error = "";
    $country = "No perfect money address";
  } else {
...
$mail->Subject = 'Verification Email: Crystals Exchange Plc ';
...
$mail->Body="
<p style='text-align:center;';> <img alt='PHPMailer' src='cid:logo'>rystalsexchange</p>
<p align=left> 
Hello $first_name $last_name, 
Thank you for registering with Crystal Exchange Plc 
<br/>
Email address: $email
<br>
Password : $password
...
if (!$mail->send()) {
  ...
  <strong>Fail</strong> Registeration failed, please try again later
     ...
     $insert_user = "INSERT INTO users (first_name, last_name , email , wallet , username , country , referal_code , password_user , ip , token , id_no , promo_code ,verified,Status) 
     VALUES ('$first_name','$last_name','$email','$btc_address', '$username ' , '$country', '$referal_code', '$password', '$ip', '$token', '$id_no', '$promo', '$verified','$status' )";
